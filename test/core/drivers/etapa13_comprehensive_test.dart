...
void main() {
  group('Etapa 13 Comprehensive Tests', () {
    test('ManufacturerDriverContractTest: Teltonika, Suntech, DefaultDriver',
      ...
      final normTel = teltonika.normalize(
          inputTel,
          DeviceContext(
              deviceId: '1',
              identity: idTel,
              capabilities: teltonika.capabilities(DeviceContext(
                  deviceId: '1',
                  identity: idTel,
    ...
    test(
        'CanProviderTest: unsupported, waiting, detected, receiving, stale, error',
        () {
      final unsuppSession = DeviceSession(
        id: '1',
        identity: const DeviceIdentity(
            id: '1', manufacturer: Manufacturer.suntech, confidence: 100),
        capabilities: const DeviceCapabilities(
            can: false,
            ble: false,
            hasCan: false,
        ...
        normalizedState: NormalizedDeviceState(
          ...
          vehicle: const VehicleState(
              ignition: true, movement: true, speedKph: 50, odometerKm: 1000),
          power: const PowerState(
              externalVoltage: 14.0,
              internalVoltage: 3.8,
              batteryPercent: 90,
              charging: true),
      ...
      final statusUnsupp =
          CanRuntimeStatus.fromSession(deviceId: '1', session: unsuppSession);
      expect(statusUnsupp.supportsCan, false);

      final canCapsSession = DeviceSession(
        id: '2',
        identity: const DeviceIdentity(
            id: '2', manufacturer: Manufacturer.teltonika, confidence: 100),
        capabilities: const DeviceCapabilities(
            can: true,
            ble: false,
            hasCan: true,
      ...
      final statusWaiting =
          CanRuntimeStatus.fromSession(deviceId: '2', session: canCapsSession);
      ...
      final statusReceiving = CanRuntimeStatus.fromSession(
          deviceId: '2', session: receivingSession);
      expect(statusReceiving.isReceiving, true);
    });

    test(
        'DeviceSessionTest: merge, deduplicação, append, restore, raw data preservado',
        () {
      const identity = DeviceIdentity(
          id: 'dev1', manufacturer: Manufacturer.teltonika, confidence: 100);
      ...
      final m1 = NormalizedMeasurement(
          key: 'rpm',
          rawKey: 'RPM',
          category: 'can',
          name: 'RPM',
          value: 1000,
          timestamp: DateTime.now());
      final m2 = NormalizedMeasurement(
          key: 'rpm',
          rawKey: 'RPM',
          category: 'can',
          name: 'RPM',
    ...
    test('ConfigurationWorkflowTest & DeviceSessionMigrationTest placeholders',
      ...
      expect(
          TeltonikaDriver().configuration(const DeviceContext(
              deviceId: '1',
          ...
          isNotEmpty);
      expect(
          SuntechDriver().configuration(const DeviceContext(
              deviceId: '2',
              identity: DeviceIdentity(
