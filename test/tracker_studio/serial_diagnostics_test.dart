...
void main() {
  test('calculates printable ASCII percentage', () {
    ...
    expect(printableAsciiRatio(Uint8List.fromList([0x01, 0xFF, 0x04, 0x41])), 0.25);
  });
  ...
  test('matrix creates all route baud ending and command combinations', () {
    ...
    expect(matrix.any((item) => item.commandPortPath.endsWith('11200') && item.readPortPath.endsWith('11202')), isTrue);
    expect(matrix.any((item) => item.commandPortPath.endsWith('11202') && item.readPortPath.endsWith('11200')), isTrue);
  });
  ...
  test('binary bytes are not recognized as probable ASCII', () {
    ...
    expect(isProbableAsciiResponse(String.fromCharCodes(bytes), bytes: bytes), isFalse);
  });
}
